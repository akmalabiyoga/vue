<template>
  <div class="container-xxl">
    <div class="d-flex justify-content-between align-items-center my-3 mx-2">
      ...
      <div>
        <button class="btn btn-sm btn-primary me-2" @click="fetchDataUserList">Fetch Data User List</button>
    ...
    <div>
      <div class="card">
        <div class="card-body">
          ...
          <pre v-if="dataUserList">{{ JSON.stringify(dataUserList, null, 2) }}</pre>
          <p v-else>No user list fetched yet. Click the button above to fetch user list.</p>
        </div>
      </div>
    </div>
    <!-- <pre>{{ apiData }}</pre> -->
...
async function fetchDataUserList() {
  try {
